...
def fun_mean_fitness(x):
    ...
    kappa = x[0]
    x_mean = x[1]  
    read_num_t2_est = (read_num_t1_global * np.exp(np.log(1/(1+x_mean)) * (t_seq_t1t2_global[1] - t_seq_t1t2_global[0])) *
                       read_depth_seq_t1t2_global[1] /read_depth_seq_t1t2_global[0])
